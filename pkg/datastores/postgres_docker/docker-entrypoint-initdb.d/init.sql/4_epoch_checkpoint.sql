...
DECLARE
BEGIN
    UPDATE validators_epoch_checkpoint vc SET validators_balances_recorded = (SELECT validators_balances_recorded_at_epoch(checkpoint_epoch)), validators_active = (SELECT validators_active_at_epoch(checkpoint_epoch)) WHERE vc.validators_balance_epoch = checkpoint_epoch;
END;
$BODY$
LANGUAGE plpgsql VOLATILE
COST 100;

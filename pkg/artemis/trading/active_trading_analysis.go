...
func ApplyMaxTransferTax2(ctx context.Context, tf *web3_client.TradeExecutionFlow) error {
	...
	if !tf.AreAllTradesValid() {
		log.Warn().Msg("ApplyMaxTransferTax2: trades are not valid")
		return errors.New("ApplyMaxTransferTax2: trades are not valid")
	}

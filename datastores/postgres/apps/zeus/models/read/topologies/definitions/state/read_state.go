...
func (s *ReadDeploymentStatusesGroup) readClustersDeployedToKns() sql_query_templates.QueryParams {
	...
	query := `SELECT MAX(td.topology_id) as topology_id, topology_system_component_name, topology_skeleton_base_name
				FROM topologies_deployed td
				JOIN topologies_kns kns ON kns.topology_id = td.topology_id
				JOIN org_users_topologies out ON out.topology_id = td.topology_id
				JOIN topology_infrastructure_components tip ON tip.topology_id = td.topology_id
...
func (s *ReadDeploymentStatusesGroup) ReadLatestDeployedClusterTopologies(ctx context.Context, cctx zeus_common_types.CloudCtxNs, ou org_users.OrgUser) error {
	...
	for rows.Next() {
		...
		rowErr := rows.Scan(
			&rs.TopologyID, &rs.ClusterName, &rs.TopologyName,
		)

...
func DeleteK8sWorkload(ctx context.Context, kns autok8s_core.KubeCtxNs, nk chart_workload.NativeK8s) error {
	...
	if nk.Service != nil {
		err := zeus.K8Util.DeleteServiceWithKns(ctx, kns, nk.Service.Name, nil)
		if err != nil {
			return err
		}
	}
	if nk.ConfigMap != nil {
		err := zeus.K8Util.DeleteConfigMapWithKns(ctx, kns, nk.ConfigMap.Name, nil)
		if err != nil {
			return err
		}
	}
	if nk.Ingress != nil {
		err := zeus.K8Util.DeleteIngressWithKns(ctx, kns, nk.Ingress.Name, nil)
		if err != nil {
			return err
		}
	}

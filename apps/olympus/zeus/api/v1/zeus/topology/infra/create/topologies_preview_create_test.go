...
const (
	previewEndpoint = "/infra/ui/cluster/preview"
	createEndpoint  = "/infra/ui/cluster/create"
)

func (t *TopologyPreviewCreateClassRequestTestSuite) TestCreateCluster() {
	t.InitLocalConfigs()

	t.Eg.POST(createEndpoint, PreviewCreateTopologyInfraActionRequestHandler)
	...
	req := zeus_templates.Cluster{
		ClusterName:     "avaxNodeTest" + rand.String(10),
...
func (t *TopologyPreviewCreateClassRequestTestSuite) TestGeneratePreview() {
	...
	t.Eg.POST(previewEndpoint, PreviewCreateTopologyInfraActionRequestHandler)
	start := make(chan struct{}, 1)
	go func() {
		close(start)
		_ = t.E.Start(":9010")

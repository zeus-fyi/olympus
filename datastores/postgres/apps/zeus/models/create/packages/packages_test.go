...
func (p *PackagesTestSuite) TestInsertRealApp() {
	ns := sql.NullString{}
	...
	filepath := p.TestDirectory + "/apps/eth-indexer/deployment.yaml"
	...
	filepath = p.TestDirectory + "/apps/eth-indexer/service.yaml"
	jsonBytes, err = p.Yr.ReadYamlConfig(filepath)
	...
	filepath = p.TestDirectory + "/apps/eth-indexer/ingress.yaml"
	jsonBytes, err = p.Yr.ReadYamlConfig(filepath)

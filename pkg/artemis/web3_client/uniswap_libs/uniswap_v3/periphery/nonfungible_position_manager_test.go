...
func TestAddCallParameters(t *testing.T) {
	...
	opts = &AddLiquidityOptions{
		...
		CommonAddLiquidityOptions: &CommonAddLiquidityOptions{
			SlippageTolerance: slippageToleranceT,
			Deadline:          deadlineT,
			UseNative:         uniswap_core_entities.EtherOnChain(1),
		},
	}
	_, err = AddCallParameters(pos, opts)
...
func TestCollectCallParameters(t *testing.T) {
	...
	opts := &CollectOptions{
		TokenID:               tokenIDT,
		ExpectedCurrencyOwed0: uniswap_core_entities.FromRawAmount(token0T, big.NewInt(0)),
		ExpectedCurrencyOwed1: uniswap_core_entities.FromRawAmount(token1T, big.NewInt(0)),
		Recipient:             recipientT,
	}
	...
	opts = &CollectOptions{
		TokenID:               tokenIDT,
		ExpectedCurrencyOwed0: uniswap_core_entities.FromRawAmount(token1T, big.NewInt(0)),
		ExpectedCurrencyOwed1: uniswap_core_entities.FromRawAmount(ether, big.NewInt(0)),
...
func TestRemoveCallParameters(t *testing.T) {
	...
	opts := &RemoveLiquidityOptions{
		TokenID:             tokenIDT,
		LiquidityPercentage: uniswap_core_entities.NewPercent(big.NewInt(1), big.NewInt(1)),
		SlippageTolerance:   slippageToleranceT,
		Deadline:            deadlineT,
	...
	opts = &RemoveLiquidityOptions{
		TokenID:             tokenIDT,
		LiquidityPercentage: uniswap_core_entities.NewPercent(big.NewInt(1), big.NewInt(100)),
		SlippageTolerance:   slippageToleranceT,
		Deadline:            deadlineT,
		CollectOptions: &CollectOptions{
			ExpectedCurrencyOwed0: uniswap_core_entities.FromRawAmount(token0T, big.NewInt(0)),
	...
	opts = &RemoveLiquidityOptions{
		TokenID:             tokenIDT,
		LiquidityPercentage: uniswap_core_entities.NewPercent(big.NewInt(99), big.NewInt(100)),
		SlippageTolerance:   slippageToleranceT,
		Deadline:            deadlineT,
		BurnToken:           true,
		CollectOptions: &CollectOptions{
	...
	opts = &RemoveLiquidityOptions{
		TokenID:             tokenIDT,
		LiquidityPercentage: uniswap_core_entities.NewPercent(big.NewInt(1), big.NewInt(2)),
		SlippageTolerance:   slippageToleranceT,
		Deadline:            deadlineT,
		CollectOptions: &CollectOptions{
			ExpectedCurrencyOwed0: uniswap_core_entities.FromRawAmount(token0T, big.NewInt(0)),
	...
	// works with eth
	ethAmount := uniswap_core_entities.FromRawAmount(uniswap_core_entities.EtherOnChain(1), big.NewInt(0))
	tokenAmount := uniswap_core_entities.FromRawAmount(token1T, big.NewInt(0))
	var (
		owed0Amount *uniswap_core_entities.CurrencyAmount
		owed1Amount *uniswap_core_entities.CurrencyAmount
		owed0Token  *uniswap_core_entities.Token
		owed1Token  *uniswap_core_entities.Token
	)
	if pool1wethT.Token0.Equal(token1T) {
		owed0Amount = tokenAmount
		owed1Amount = ethAmount
		owed0Token = token1T
		owed1Token = uniswap_core_entities.EtherOnChain(1).Wrapped()
	} else {
		owed0Amount = ethAmount
		owed1Amount = tokenAmount
		owed0Token = uniswap_core_entities.EtherOnChain(1).Wrapped()
		owed1Token = token1T
	}
	pos, err = entities.NewPosition(pool01T, big.NewInt(100), -constants.TickSpacings[constants.FeeMedium], constants.TickSpacings[constants.FeeMedium])
	assert.NoError(t, err)

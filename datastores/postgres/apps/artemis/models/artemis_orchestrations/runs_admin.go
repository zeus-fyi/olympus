...
func UIAdminSelectAiSystemOrchestrations(ctx context.Context, ou org_users.OrgUser, rid int) ([]OrchestrationsAnalysis, error) {
	...
	*/
	...
	q.RawQuery = `WITH cte_a AS (
						...
						WHERE 
							$1::int8 != 0 AND o.org_id != 1685378241971196000 AND ar.is_archived = false` + queryByRunID + ` 
						ORDER BY
...
func AdminSelectAiSystemOrchestrations(ctx context.Context, ou org_users.OrgUser, rid int) ([]OrchestrationsAnalysis, error) {
	var ojs []OrchestrationsAnalysis
	q := sql_query_templates.QueryParams{}
	args := []interface{}{ou.OrgID}

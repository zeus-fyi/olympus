...
func SwapCallParameters(trades []*entities.Trade, options *SwapOptions) (*utils.MethodParameters, error) {
	...
	ZeroIn := uniswap_core_entities.FromRawAmount(trades[0].InputAmount().Currency, big.NewInt(0))
	ZeroOut := uniswap_core_entities.FromRawAmount(trades[0].OutputAmount().Currency, big.NewInt(0))
	...
	mustRefund := sampleTrade.InputAmount().Currency.IsNative() && sampleTrade.TradeType == uniswap_core_entities.ExactOutput
	...
	for _, trade := range trades {
		for _, swap := range trade.Swaps {
			...
			if singleHop {
				if trade.TradeType == uniswap_core_entities.ExactInput {
				...
				path, err := EncodeRouteToPath(swap.Route, trade.TradeType == uniswap_core_entities.ExactOutput)
				if err != nil {
					return nil, err
				}

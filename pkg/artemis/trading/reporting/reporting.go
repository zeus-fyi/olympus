...
func getQ(blockCheckpoint int) string {
	var que = fmt.Sprintf(`WITH cte_failed_count AS (
			...
			AND rx_block_number > %d AND amount_in_addr = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2'
			GROUP BY amount_out_addr
				...
				WHERE end_reason = 'success' AND rx_block_number > %d AND expected_profit_amount_out IS NOT NULL AND amount_in_addr = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2'
			`, blockCheckpoint, blockCheckpoint)
	return que
}

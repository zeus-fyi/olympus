...
func (c *DestroyClusterSetupWorkflow) DestroyClusterSetupWorkflowFreeTrial(ctx workflow.Context, wfID string, params base_deploy_params.DestroyClusterSetupRequest) error {
	...
	if params.FreeTrial {
		...
		if !isBillingSetup {
			...
			if err != nil {
				log.Error("Failed to select gke free trial nodes", "Error", err)
				return err
			}
			...
			if err != nil {
				log.Error("Failed to select eks free trial nodes", "Error", err)
				return err
			}
			for _, node := range eksNodes {
				...
				if err != nil {
					log.Error("Failed to remove eks node resources for account", "Error", err)
					return err
				}
			}

...
func (a *AthenaClient) Resume(ctx context.Context, rr athena_routines.RoutineRequest) error {
	a.PrintReqJson(rr)
	resp, err := a.R().
		SetBody(rr).
		Post(athena_endpoints.InternalResumeV1Path)

	if err != nil || resp.StatusCode() != http.StatusOK {
		log.Ctx(ctx).Err(err).Msg("AthenaClient: Resume")

...
func (c *DestroyNamespaceSetupWorkflow) DestroyNamespaceSetupWorkflow(ctx workflow.Context, params base_deploy_params.TopologyWorkflowRequest) error {
	...
	err = workflow.ExecuteActivity(removeSubdomainCtx, c.CreateSetupTopologyActivities.RemoveDomainRecord, params.Namespace).Get(removeSubdomainCtx, nil)
	if err != nil {
	...
	if len(disks) > 0 {
		...
		for i, disk := range disks {
			resourceIDs[i] = disk.Disks.ResourceID
		}

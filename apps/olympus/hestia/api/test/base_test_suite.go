...
type HestiaBaseTestSuite struct {
	E  *echo.Echo
	Eg *echo.Group
...
func (t *HestiaBaseTestSuite) SetupTest() {
	t.InitLocalConfigs()
	t.E = echo.New()
	t.Eg = t.E.Group("/")
	...
	t.ZeusClient = zeus_client.NewZeusClient("http://localhost:9002", t.Tc.ProductionLocalTemporalBearerToken)
}

func (t *HestiaBaseTestSuite) PostRequest(ctx context.Context, endpoint string, payload any) ([]byte, error) {
	...
	if err != nil || resp.StatusCode() != http.StatusOK {
		log.Ctx(ctx).Err(err).Msg("HestiaBaseTestSuite: PostRequest")

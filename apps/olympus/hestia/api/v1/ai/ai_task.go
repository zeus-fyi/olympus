...
func SupportEmailAIServiceTaskRequestHandler(c echo.Context) error {
	...
	return request.SupportAcknowledgeAITask(c)
}

func (a *AIServiceRequest) SupportAcknowledgeAITask(c echo.Context) error {
	log.Info().Msg("Hestia: CreateAIServiceTaskRequestHandler")
	//content := ""
	//ou := org_users.OrgUser{}
	//key := read_keys.NewKeyReader()
	...
	//	return c.JSON(http.StatusBadRequest, nil)
	//}
	msgs, err := hermes_email_notifications.SupportEmailUser.GetReadEmails("support@zeus.fyi")
	if err != nil {

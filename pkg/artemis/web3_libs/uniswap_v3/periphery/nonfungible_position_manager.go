...
type CommonAddLiquidityOptions struct {
	SlippageTolerance *uniswap_core_entities.Percent // How much the pool price is allowed to move
	Deadline          *big.Int                       // When the transaction expires, in epoch seconds
	UseNative         *uniswap_core_entities.Ether   // Whether to spend ether. If true, one of the pool tokens must be WETH, by default false
...
type CollectOptions struct {
	...
	ExpectedCurrencyOwed0 *uniswap_core_entities.CurrencyAmount // Expected value of tokensOwed0, including as-of-yet-unaccounted-for fees/liquidity value to be burned
...
type RemoveLiquidityOptions struct {
	...
	LiquidityPercentage *uniswap_core_entities.Percent // The percentage of position liquidity to exit
	SlippageTolerance   *uniswap_core_entities.Percent // How much the pool price is allowed to move
	Deadline            *big.Int                       // When the transaction expires, in epoch seconds.
...
func encodeCollect(opts *CollectOptions) ([][]byte, error) {
	...
	if involvesETH {
		var (
			ethAmount   *big.Int
			token       *uniswap_core_entities.Token
			tokenAmount *big.Int
		)
...
func RemoveCallParameters(position *entities.Position, opts *RemoveLiquidityOptions) (*utils.MethodParameters, error) {
	...
	partialPosition, err := entities.NewPosition(
		position.Pool,
		opts.LiquidityPercentage.Multiply(uniswap_core_entities.NewPercent(position.Liquidity, big.NewInt(1))).Quotient(),
		position.TickLower,
		position.TickUpper,
	)
	if err != nil {
	...
	collectOpts := &CollectOptions{
		...
		ExpectedCurrencyOwed0: opts.CollectOptions.ExpectedCurrencyOwed0.Add(uniswap_core_entities.FromRawAmount(opts.CollectOptions.ExpectedCurrencyOwed0.Currency, amount0Min)),
		ExpectedCurrencyOwed1: opts.CollectOptions.ExpectedCurrencyOwed1.Add(uniswap_core_entities.FromRawAmount(opts.CollectOptions.ExpectedCurrencyOwed1.Currency, amount1Min)),
	...
	if opts.LiquidityPercentage.EqualTo(uniswap_core_entities.NewFraction(constants.One, big.NewInt(1))) {
		if opts.BurnToken {

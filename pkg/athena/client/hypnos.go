...
func (a *AthenaClient) Hypnos(ctx context.Context, rr athena_routines.RoutineRequest) error {
	a.PrintReqJson(rr)
	resp, err := a.R().
		SetBody(rr).
		Post(athena_endpoints.InternalHypnosV1Path)

	if err != nil || resp.StatusCode() != http.StatusOK {
		log.Ctx(ctx).Err(err).Msg("AthenaClient: Hypnos")

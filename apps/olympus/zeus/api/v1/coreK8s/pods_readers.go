...
func PodsDescribeRequest(c echo.Context, request *PodActionRequest) error {
	...
	pods, err := zeus_pkg.K8Util.GetPodsUsingCtxNs(ctx, request.Kns, request.LogOpts, request.FilterOpts)
	if err != nil {
		return err
	}
...
func PodLogsActionRequest(c echo.Context, request *PodActionRequest) error {
	...
	logs, err := zeus_pkg.K8Util.GetPodLogs(ctx, p.GetName(), request.Kns.Namespace, request.LogOpts, request.FilterOpts)
	if err != nil {
		return err
	}

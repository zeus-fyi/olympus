...
func (t *TradeExecutionFlow) AreAllTradesValid() bool {
	...
	if !t.FrontRunTrade.AreTradeParamsValid() {
		log.Warn().Str("amountIn", t.FrontRunTrade.AmountIn.String()).Str("amountOut", t.FrontRunTrade.AmountOut.String()).Msg("TradeExecutionFlow: front trade is not valid")
		log.Warn().Msg("TradeExecutionFlow: front run trade is not valid")
		return false
	}
	if !t.UserTrade.AreTradeParamsValid() {
		log.Warn().Str("amountIn", t.UserTrade.AmountIn.String()).Str("amountOut", t.UserTrade.AmountOut.String()).Msg("TradeExecutionFlow: user trade is not valid")
		log.Warn().Msg("TradeExecutionFlow: user trade is not valid")
		return false
	}
	if !t.SandwichTrade.AreTradeParamsValid() {
		log.Warn().Str("amountIn", t.SandwichTrade.AmountIn.String()).Str("amountOut", t.SandwichTrade.AmountOut.String()).Msg("TradeExecutionFlow: sandwich trade is not valid")

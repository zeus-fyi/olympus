...
func QuoteCallParameters(
	...
	tradeType uniswap_core_entities.TradeType,
	...
	if singleHop {
		if tradeType == uniswap_core_entities.ExactInput {
		...
		path, err := EncodeRouteToPath(route, tradeType == uniswap_core_entities.ExactOutput)
		if err != nil {
			return nil, err
		}
		if tradeType == uniswap_core_entities.ExactInput {
...
func EncodeRouteToPath(route *entities.Route, exactOutput bool) ([]byte, error) {
	...
	for i, pool := range route.Pools {
		var outputToken *uniswap_core_entities.Token
